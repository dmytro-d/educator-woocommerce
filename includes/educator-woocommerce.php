...
function edu_wc_get_objects_by_product( $product_id ) {
	return Educator_WooCommerce::get_instance()->get_objects_by_product( $product_id );
}
...
class Educator_WooCommerce {
	...
	protected function __construct() {
		$this->pt_course = EDR_PT_COURSE;
		$this->pt_membership = EDR_PT_MEMBERSHIP;

		add_action( 'plugins_loaded', array( $this, 'configure' ) );

		// Add the "wc_order" entry origin to mark WooCommerce generated entries.
		add_filter( 'edr_entry_origins', array( $this, 'entry_origins' ) );

		// Filter buy widgets.
		add_filter( 'edr_get_course_buy_widget_pre', array( $this, 'filter_course_buy_widget' ), 10, 4 );
		add_filter( 'edr_get_membership_buy_widget_pre', array( $this, 'filter_membership_buy_widget' ), 10, 4 );
	...
	public function get_pt_course() {
		return $this->pt_course;
	}

	public function get_pt_membership() {
		return $this->pt_membership;
	}

	public function get_sold_post_types() {
		return array( $this->pt_course, $this->pt_membership );
	}
	...
	/**
	 * Filter the course buy widget.
	 *
	 * @param string $output
	 * @param array $atts
	 * @param int $user_id
	 * @param bool $membership_allows
	 * @return string
	 */
	public function filter_course_buy_widget( $output, $atts, $user_id, $membership_allows ) {
	...
	/**
	 * Filter the membership buy widget.
	 *
	 * @param string $output
	 * @param array $atts
	 * @param int $user_id
	 * @return string
	 */
	public function filter_membership_buy_widget( $output, $atts, $user_id ) {
		$product = $this->get_object_product( $atts['object_id'] );
	...
	public function get_user_entries( $order ) {
		...
		$order_entries = array();
		$tmp = $this->get_entries( array( 'user_id' => $order->user_id ) );
	...
	public function one_membership_in_cart( $valid, $product_id ) {
		...
		if ( ! empty( $objects ) ) {
			...
			foreach ( $objects as $object ) {
				if ( $this->pt_membership == $object->post_type ) {
